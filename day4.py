...
def check_letter (x_d,y_d, letter, list_out, compass):
    if -1< x_d < int(n_columns-1) and -1 < y_d < int(n_rows-1) and content[int(x_d)] [int(y_d)] == letter:
...
def check_letter_direction (X_D, Y_D, letter, list_out, compass):
    if -1 < X_D < int(n_columns-1) and -1 < Y_D < int(n_rows-1) and direction == compass and content[int(X_D)][int(Y_D)] == letter:
